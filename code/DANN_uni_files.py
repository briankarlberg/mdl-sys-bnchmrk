...
if __name__ == '__main__':
    ...
    whole_data: pd.DataFrame = pd.read_csv(data_path, sep='\t')

    for cancer in whole_data[cancer_column].unique():
        val_data = whole_data[whole_data[cancer_column] == cancer]
        # select all cacners except the one we are currently working on
        data = whole_data[whole_data[cancer_column] != cancer]
        ...
        val_cancer_labels = val_data[[cancer_column]]
        val_systems_labels = val_data[[systems_column]]
        val_sample_ids = val_data['improve_sample_id']
        ...
        val_data = val_data.drop(columns=[cancer_column, systems_column, 'improve_sample_id'])
        ...
        encoded_val_cancer_labels = cancer_le.transform(val_cancer_labels)
        ...
        encoded_val_systems_labels = systems_le.transform(val_systems_labels)
        ...
        domain_invariant_features = feature_extractor.predict(val_data)
        ...
        domain_invariant_features_df.insert(0, cancer_column, val_data[cancer_column])
        domain_invariant_features_df.insert(0, systems_column, val_data[systems_column])
        domain_invariant_features_df.index = val_data['improve_sample_id']

        # colon-adeno_transcriptomics_cell-line+CPTAC.vae-out.250-ltnt-dim_12-epchs.tsv
        domain_invariant_features_df.to_csv(
            Path(output_dir, f"{data_path.stem}_{cancer}_latent_space.dann-out.{latent_space}-ltnt-dim.tsv"), sep='\t',
            index=True)

        # save history
        history = pd.DataFrame(history.history)
        history.to_csv(Path(output_dir, f"{data_path.stem}_{cancer}_history.dann-out.{latent_space}-ltnt-dims.tsv"), sep='\t',
                       index=True)
