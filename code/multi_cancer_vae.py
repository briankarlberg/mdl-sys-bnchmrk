...
class VAE(models.Model):
    ...
    def train_step(self, data):
        features, cancer_train_data, systems_train_data = data
        with tf.GradientTape() as vae_tape, tf.GradientTape() as disc_tape:
            ...
            # Forward pass through the classifier using latent representation
            cancer_pred = self.classifier(z)

            # Compute classification loss
            # classification_loss = tf.keras.losses.categorical_crossentropy(cancer_data, cancer_pred)
            classification_loss = 3 * tf.reduce_mean(
                tf.keras.losses.categorical_crossentropy(cancer_train_data, cancer_pred, from_logits=False))

            # Discriminator loss
            system_pred = self.discriminator(z)
            discriminator_loss = 0.5 * BinaryCrossentropy(from_logits=False)(tf.zeros_like(system_pred), system_pred)

            # Adjusted VAE loss
            total_loss = vae_loss - discriminator_loss + classification_loss

            # vae_loss = vae_loss + tf.reduce_mean(classification_loss) - tf.reduce_mean(discriminator_loss)
...
if __name__ == '__main__':
    ...
    cancer_classifier = build_cancer_classifier(latent_dim=latent_space_dim, num_classes=len(np.unique(s1_cancer_enc)))
    ...
    reducer = umap.UMAP()
    embedding = reducer.fit_transform(z_df)

    # plot the umap
    import matplotlib.pyplot as plt
    import seaborn as sns

    sns.scatterplot(x=embedding[:, 0], y=embedding[:, 1], hue=cancer_data, palette="viridis", style=system_data)
